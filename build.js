...
function extractProps(page) {
  ...
  const title =
    p.Title?.title?.map((t) => t.plain_text).join("") || "Untitled";
  const brand =
    p.Brand?.multi_select?.map((s) => s.name).join(", ") || "";
  const model = p.Model?.rich_text?.map((t) => t.plain_text).join("") || "";
  const year = p["Manufactured Year"]?.number || null;
  const km = p.Km?.number || null;
  const price = p.Price?.number || null;
  const fuel = p.Fuel?.select?.name || "";
  const transmission = p.Transmission?.select?.name || "";
  const color = p.Color?.select?.name || "";
  const state = p.State?.select?.name || "";
  const equipment =
    p["Details/Equipment"]?.multi_select?.map((s) => s.name) || [];
  const photos = p.Photos?.files || [];
...
async function main() {
  console.log("Fetching stock from Notion...");
  const pages = await queryStock();
  console.log(`Found ${pages.length} available vehicles.`);

  fs.mkdirSync(IMAGES_DIR, { recursive: true });

  const vehicles = [];
  for (const page of pages) {
    const v = extractProps(page);
    ...
    if (v.photos.length > 0) {
      ...
      if (url) {
        ...
        console.log(`Downloading image for ${v.title}...`);
        v.localImage = await downloadImage(url, filename);
      }
    }
  ...
  const html = buildHTML(vehicles);
  fs.writeFileSync(path.join(OUTPUT_DIR, "index.html"), html);
  console.log("Built index.html");

  const src = path.join(__dirname, "src");
  fs.copyFileSync(path.join(src, "style.css"), path.join(OUTPUT_DIR, "style.css"));
  fs.copyFileSync(path.join(src, "app.js"), path.join(OUTPUT_DIR, "app.js"));
  console.log("Copied static assets.");

  console.log(`Done! ${vehicles.length} vehicles published.`);
}
